...
#### `#绑定备份群 <群号> [--to <dir>] [--inc|--full] [--flat|--keep]`
...
1. 在配置 `groupSyncTargets` 中 **upsert** 该群：
   ...
   - `mode/flat`：默认走 `groupSyncDefaults`，可用参数覆盖
2. 回复“绑定成功”图片卡：
   - 展示该群关键策略（目标目录、模式、并发/超时等取最终生效值）
   - 给出下一步快捷命令：`#同步群文件 <群号> --inc|--full`、`#解绑备份群 <群号>` 等

> 备注：当前代码里，如果群不在 `groupSyncTargets`，手动 `#同步群文件` 默认会走全量；绑定后可明确落在配置里，默认就能走增量（由 defaults/target 决定）。
