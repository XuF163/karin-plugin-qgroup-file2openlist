...
- 群文件同步到 OpenList（私聊指令）：`#同步群文件 <群号> [--to <dir>] [--flat|--keep] [--timeout <sec>] [--full|--inc] [--folder <id>] [--max <n>] [--concurrency <n>]`
- 群同步配置（私聊/主人权限）：`#群同步配置 列表|查看|添加|删除|启用|停用|...`
- 定时同步调度（后台任务）：按 `groupSyncTargets[].schedule.cron` 命中触发，并受 `timeWindows` 限制
- OpenList → OpenList 备份（私聊指令）：`#备份oplist <源OpenList地址> [--src] [--to] [--inc|--full] [--api|--webdav|--auto] ...`
...
src/
  ...
  apps/                       # “薄层”：命令匹配/参数解析/权限/回复消息
    ...
    groupSyncScheduler.ts     # 每秒 tick 的调度任务（检查 cron/timeWindows）
...
- `urlConcurrency` / `transferConcurrency` / `fileTimeoutSec` / `retryTimes` / `retryDelayMs` / `progressReportEvery` / `downloadLimitKbps` / `uploadLimitKbps`：逐项覆盖默认策略
- `uploadBackup`：是否启用“群文件上传事件自动备份”
- `timeWindows`：时段限制（仅用于定时任务/自动任务），例如 `00:00-06:00,23:00-23:59`
- `schedule`：定时同步配置
  - `enabled`
  - `cron`：cron 表达式（支持 5/6 段；6 段为 `秒 分 时 日 月 周`）
...
参数（命令参数优先级最高，其次 `groupSyncTargets[groupId]`，最后 `groupSyncDefaults`）：
...
- 如果 **该群在 `groupSyncTargets` 里存在配置**：同步模式会按 `target.mode` → `groupSyncDefaults.mode` → `incremental` 兜底。
- 如果 **该群没有配置在 `groupSyncTargets`**：手动 `#同步群文件` 会默认使用 **全量**（`full`），其它参数（并发/超时/平铺/限速）仍会按 `groupSyncDefaults` 兜底。
  - 也就是说：想让某个群“默认增量”，请把该群加入 `groupSyncTargets` 并设置 `mode=incremental`（可用 WebUI 或 `#群同步配置 <群号> 添加`）。
...
- `#群同步配置 <群号> 计划 <cron>` 或 `#群同步配置 <群号> 计划 开/关`

### 6.4 定时同步调度（后台任务）

入口：`src/apps/groupSyncScheduler.ts` → `src/model/groupSync/scheduler.ts`

- 任务：每秒 tick 一次（cron：`* * * * * *`）
- 行为：
  1. 遍历 `groupSyncTargets`
  2. 对启用且 `schedule.enabled === true` 且 `cron` 命中的群触发同步
  3. 如果配置了 `timeWindows`，并且当前时间不在允许时段内，则跳过
  4. 同一秒内同一 `(groupId, cron)` 只触发一次（`lastTriggered` 去重）

cron 解析能力（见 `cronMatches`）：

- 支持 5 段或 6 段 cron（5 段会默认补 `秒=0`）
- 字段支持：`*`、`?`、单值、范围（`1-5`）、步长（`*/5`）、列表（`,`）
- 周字段：`0` 或 `7` 表示周日
...
- `help.html` 中的示例参数可能与真实解析参数存在差异（例如模板里出现 `--mode=full`，而实际指令支持 `--full/--inc`）。
...
- `#绑定备份群 <群号> [--to /目标目录] [--inc|--full] [--flat|--keep]`
...
1. 先用少量文件/短超时测试（`--max 1`、`--timeout 60`）
