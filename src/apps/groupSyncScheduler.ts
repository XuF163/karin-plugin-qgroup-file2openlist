...
import { syncGroupFilesToOpenListCore } from './groupFiles'
...
const parseTimeWindows = (value: string) => {
...
const isNowAllowed = (timeWindows: unknown, now: Date) => {
...
export const groupFileSyncScheduler = karin.task('群文件同步调度器', '* * * * * *', async () => {
  const cfg = config()
  const targets = Array.isArray(cfg.groupSyncTargets) ? cfg.groupSyncTargets : []
  const now = new Date()
