...
/**
 * 群同步配置命令的核心处理逻辑（apps 层仅做命令注册）。
 */
export const handleGroupSyncConfigCommand = async (e: any) => {
  if (!e.isPrivate) return false

  const raw = String(e.msg ?? '').replace(/^#?(群同步配置|同步群配置|群文件同步配置)/i, '').trim()
  ...
  if (action === 'concurrency' || action === 'urlConcurrency') {
    ...
    if (!value || value <= 0) {
      await e.reply('并发参数错误，请输入大于 0 的整数。')
      return true
    }
  ...
  // 理论上不会走到这里
  logger.debug(`[groupSyncConfig] unknown action: ${action}`)
  await e.reply(cfgHelp)
  return true
}
